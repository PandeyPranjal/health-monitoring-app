      ...
      <div className="relative w-full max-w-sm bg-surface rounded-t-[var(--radius-xl)] sm:rounded-[var(--radius-xl)]
        ...
        {isSuccess ? (
          <div className="animate-scale-in text-center py-6">
            <div className="w-20 h-20 bg-success/10 rounded-full flex items-center justify-center mx-auto mb-6 bounce-in">
              <CheckIcon className="w-10 h-10 text-success" />
            </div>
            <h3 className="text-xl font-bold text-text-primary mb-2">Booking Confirmed</h3>
            <p className="text-sm text-text-muted mb-8 px-4">
              Your appointment with Dr. {doctor.name} is set. Don't forget to add it to your calendar!
            </p>
            <div className="space-y-3">
              <a 
                href={`data:text/calendar;charset=utf8,BEGIN:VCALENDAR%0AVERSION:2.0%0ABEGIN:VEVENT%0ASUMMARY:Appointment with Dr. ${doctor.name}%0ADESCRIPTION:HealthPulse Online Consultation%0AEND:VEVENT%0AEND:VCALENDAR`} 
                download="appointment.ics"
                className="w-full flex items-center justify-center py-3.5 bg-surface-elevated text-primary text-[11px] font-bold uppercase tracking-widest rounded-xl hover:bg-primary/10 transition-colors border border-border shadow-sm"
              >
                Add to Apple Calendar
              </a>
              <a 
                href={`data:text/calendar;charset=utf8,BEGIN:VCALENDAR%0AVERSION:2.0%0ABEGIN:VEVENT%0ASUMMARY:Appointment with Dr. ${doctor.name}%0ADESCRIPTION:HealthPulse Online Consultation%0AEND:VEVENT%0AEND:VCALENDAR`} 
                download="google-appointment.ics"
                className="w-full flex items-center justify-center py-3.5 bg-surface-elevated text-[#DB4437] text-[11px] font-bold uppercase tracking-widest rounded-xl hover:bg-[#DB4437]/10 transition-colors border border-border shadow-sm"
              >
                Add to Google Calendar
              </a>
              <Button onClick={onSuccess} className="w-full rounded-xl mt-4">
                Done
              </Button>
            </div>
          </div>
          ...
          <>
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                className="flex-[2] rounded-xl h-12 relative overflow-hidden active:scale-[0.98] shadow-button"
              >
                {isSubmitting ? (
